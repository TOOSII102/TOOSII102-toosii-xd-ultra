...
async function run() {
    ...
    try {
        ...
        const blockedAi = checkRateLimit('ai', sender, now + 1000);
        assert.strictEqual(blockedAi.allowed, false);
        assert.strictEqual(blockedAi.retryAfterSeconds, 1);
        assert.strictEqual(checkRateLimit('ai', otherSender, now + 1000).allowed, true, 'rate limits must be per sender');

        delete require.cache[limiterPath];
        ({ checkRateLimit, resetRateLimits } = require('../lib/rateLimiter'));
        assert.strictEqual(checkRateLimit('ai', sender, now + 1500).allowed, false, 'limits must survive a module reload');
        assert.strictEqual(checkRateLimit('ai', sender, now + 2001).allowed, true, 'limits must expire after two seconds');

        resetRateLimits(true);
        assert.strictEqual(checkRateLimit('download', sender, now).allowed, true);
        assert.strictEqual(checkRateLimit('download', sender, now + 1000).allowed, false);
        assert.strictEqual(checkRateLimit('download', sender, now + 2001).allowed, true);
        ...
        try {
            ...
            assert.strictEqual(aiFetches, 1, 'blocked AI requests must not reach the provider');
            ...
            assert.strictEqual(mediaFetches, 1, 'blocked media requests must not reach a resolver');
        } finally {
