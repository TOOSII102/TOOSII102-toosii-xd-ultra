...
function getPolicy(scope) {
    ...
    if (normalized === 'ai') {
        return {
            maxRequests: positiveInteger(process.env.AI_RATE_LIMIT_MAX, 4, 1, 100),
            windowMs: positiveInteger(process.env.AI_RATE_LIMIT_WINDOW_SECONDS, 60, 5, 3600) * 1000
        };
    }
    if (normalized === 'download') {
        return {
            maxRequests: positiveInteger(process.env.MEDIA_RATE_LIMIT_MAX, 3, 1, 100),
            windowMs: positiveInteger(process.env.MEDIA_RATE_LIMIT_WINDOW_SECONDS, 60, 5, 3600) * 1000
        };
    }
