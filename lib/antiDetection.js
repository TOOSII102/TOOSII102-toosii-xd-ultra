...
class AntiDetection {
    constructor() {
        this.encryptManager = new EncryptionManager();
        this.typingSpeeds = {
    ...
    getBanRisk(phone) {
        if (!this.banRisk.has(phone)) this.banRisk.set(phone, 0);
    ...
    checkDailyLimits(phone, action) {
        ...
        if (!this.sessionData.has(key)) this.sessionData.set(key, 0);
    ...
    checkReports(phone) {
        if (!this.reportCount.has(phone)) this.reportCount.set(phone, 0);
    ...
    generateNaturalMessage(phone, context = {}) {
        ...
        if (this.config.enableEmojiUsage && Math.random() > 0.6) {
            ...
            if (Math.random() > 0.5) message = emoji + ' ' + message;
            else message = message + ' ' + emoji;
        }
        ...
        if (!this.messageHistory.has(phone)) this.messageHistory.set(phone, []);
    ...
    isRateLimited(phone, action) {
        ...
        if (action === 'message') return recent.length >= config.maxMessagesPerMinute;
        else if (action === 'call') return recent.length >= config.maxCallsPerMinute;
        return false;
    }
    ...
    async autoCleanup(sock, phone, messageId, delay = 5000) {
        try {
            ...
            console.log(`[Stealth] Auto-cleaned message ${messageId}`);
            return true;
        } catch (e) {
            console.error('[Stealth] Auto-cleanup failed:', e.message);
            return false;
        }
    }
    ...
    async sendStealthMessage(sock, phone, message, options = {}) {
        ...
        if (useNaturalTemplate && (!message || message.length < 3)) {
            const context = { previousMessages: this.messageHistory.get(phone) || [] };
            finalMessage = this.generateNaturalMessage(phone, context);
        }

        if (encryptMessage) {
            finalMessage = this.encryptManager.encryptMessage(finalMessage, encryptionMethod);
        }
        ...
        try {
            ...
            if (autoCleanup && result.key && result.key.id) {
                this.autoCleanup(sock, phone, result.key.id, cleanupDelay);
            }
    ...
    async makeStealthCall(sock, phone, options = {}) {
        ...
        try {
            const jid = phone + '@s.whatsapp.net';
            const callMessages = ['📞 Calling...', '📱 Ringing...', '🔔 Incoming call...', '📞 Missed call?', '📲 Call me back?'];
    ...
    getStats(phone = null) {
        ...
        for (const [key, timestamps] of this.rateLimits) {
            ...
            if (!stats.rateLimits[p]) stats.rateLimits[p] = { messages: 0, calls: 0 };
    ...
    resetRateLimits(phone = null) {
        if (phone) {
            for (const [key] of this.rateLimits) {
                if (key.startsWith(phone)) this.rateLimits.delete(key);
            }
    ...
    getSafeParams(action, count) {
        const config = this.config;
        let maxCount, minDelay, maxDelay;
        if (action === 'message') {
